...
class MonopolyEnv(gym.Env):
    ...
    def observation(self):
        ...
        num_features = self.num_squares * 2 + num_property_features * len(self.properties) + 4 * len(self.players)
        observation = np.zeros((num_features,), dtype=int)
        ...
        for property_index, property_info in enumerate(self.properties):
            ...
            observation[observation_idx] = property_info["color"]
            # Set property price
            observation[observation_idx + 1] = property_info["price"]
            # Set current rent
            observation[observation_idx + 2] = property_info["current_rent"]
            # Set owner (0 if unowned)
            observation[observation_idx + 3] = owner.token.number if owner is not None else 0
            # Set mortgage price
            observation[observation_idx + 4] = property_info["mortgage_price"]
            # Set whether it is mortgaged or not
            observation[observation_idx + 5] = int(is_mortgaged)
            # Set property index (for reference)
            observation[observation_idx + 6] = property_index

        # Add player-related information
        for player in self.players:
            player_id = player.id - 1
            observation_idx = self.num_squares * 2 + len(self.properties) * num_property_features + player_id * 4
            # Player's current cash
            observation[observation_idx] = player.cash
            # Whether they went to jail in this round
            observation[observation_idx + 1] = int(player.went_to_jail)
            # Whether they are currently in jail
            observation[observation_idx + 2] = int(player.in_jail)
            # Whether the player has a "Get Out of Jail Free" card
            observation[observation_idx + 3] = int(player.has_get_out_of_jail_card)
