...
class MonopolyEnv(gym.Env):
    ...
    num_property_features = 7  # Number of property-related features

    num_players = len(self.players)
    num_properties = len(self.properties)
    max_price_scale = 1e6  # Choose a very large value as the max price scale
    observation = np.zeros((num_features,), dtype=float)

    # Set player positions on the board
    for player in self.players:
        player_position = player.get_position()  # Define a get_position method for Player class
        observation[player_position * 2 + player.id - 1] = player.token.number

    # Add property-related information
    for property_index, property_info in enumerate(self.properties):
        owner = property_info.get("pieceType", None)
        is_mortgaged = property_info.get("mortgaged", False)

        observation_idx = self.num_squares * 2 + property_index * num_property_features

        # Set color (assuming color is represented as an integer)
        observation[observation_idx] = (2 * property_info["color"] / num_colors) - 1
        # Set property price (scaled)
        observation[observation_idx + 1] = (2 * property_info["price"] / max_price_scale) - 1
        # Set current rent (scaled)
        observation[observation_idx + 2] = (2 * property_info["rent"] / max_price_scale) - 1
        # Set owner (0 if unowned)
        observation[observation_idx + 3] = (2 * (owner.token.number if owner is not None else 0) / num_players) - 1
        # Set mortgage price (scaled)
        observation[observation_idx + 4] = (2 * property_info["mortgageprice"] / max_price_scale) - 1
        # Set whether it is mortgaged or not
        observation[observation_idx + 5] = 2 * int(is_mortgaged) - 1
        # Set property index (for reference)
        observation[observation_idx + 6] = (2 * property_index / num_properties) - 1

    # Add player-related information
    for player in self.players:
        player_id = player.id - 1
        observation_idx = self.num_squares * 2 + num_property_features * num_properties + player_id * 5  # +5 for the new feature
        # Player's current cash (scaled)
        observation[observation_idx] = (2 * player.cash / max_price_scale) - 1
        # Whether they went to jail in this round
        observation[observation_idx + 1] = 2 * int(player.wasJailed) - 1
        # Whether they are currently in jail
        observation[observation_idx + 2] = 2 * int(player.jail) - 1
        # Whether the player has a "Get Out of Jail Free" card
        observation[observation_idx + 3] = 2 * int(player.freeJail) - 1
        # Number of times the player has chosen to roll doubles to get out of jail free
        observation[observation_idx + 4] =  ((player.diceJail + 1 ) / 2) -1
